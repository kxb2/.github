...
기획 텍스트와 영상 제작 사이의 빈 칸을 메우는 도구 —
시나리오를 넣으면 **3×3 그리드 스토리보드 1장**과 **영상 AI에 그대로 넣는 영문 프롬프트**가 함께 나옵니다.
...
> **테스트가 있습니다.** 크롭·프롬프트 파싱·Export·캔버스 스키마 검증처럼 **손댔을 때 조용히 깨지기 쉬운 곳**이 덮여 있습니다. 아래 §9.2의 정리 작업을 할 때 안전망으로 쓰세요.
> ```bash
> pytest                    # 전체
...
| | # | 항목 | 한 줄 |
|:--:|:--:|---|---|
| 🔴 | 1 | 키 재발급 | 레포엔 없지만 **이탈 인원 전원이 보유 중** |
...
다만 개발 기간 중 Claude·OpenAI·Gemini 키를 **팀 내부에 평문으로 공유**했기 때문에, 프로젝트를 떠나는 인원 전원이 여전히 사용 가능한 키를 보유한 상태입니다.
→ **인계 시점에 전량 재발급**하면 깨끗하게 끊깁니다. 사고 대응이 아니라 인수인계 절차의 일부로 처리하면 됩니다.
...
- [ ] **Anthropic · OpenAI · Gemini API 키 전량 폐기 후 재발급** — 이탈 인원 전원이 기존 키를 보유 중 (§7-1)
- [ ] `JWT_SECRET_KEY` 재생성 (32자 이상)
- [ ] Supabase DB 비밀번호 · Cloudflare R2 액세스 키 재발급
- [ ] Google OAuth 클라이언트 시크릿 재발급 · 승인된 도메인 재확인

### 계정 · 소유권 이전

- [ ] **AWS** — EC2 인스턴스(t3.small, ap-northeast-2, Ubuntu 24.04), Organization 계정 분리 상태 확인
- [ ] **Supabase** 프로젝트 소유권
- [ ] **Cloudflare** R2 버킷
- [ ] **Vercel** 프로젝트
- [ ] **DuckDNS** 계정 (`kxb2-backend` 도메인 · 토큰) — 만료되면 백엔드 주소가 통째로 죽습니다
- [ ] **Let's Encrypt 인증서 자동 갱신** 동작 확인 (certbot 타이머)
- [ ] **GitHub** `kxb2` 조직 소유권 + Actions Secrets(`PROD_DATABASE_URL` 등)
- [ ] 각 AI 제공사 **결제 수단 · 예산 한도** 이전

### 정리

- [ ] 배포 DB의 개발/시연용 테스트 row 정리 (`storyboards` 삭제 시 `reference_images`/`generations`/`cuts`는 cascade)
- [ ] R2 버킷의 테스트 파일 정리
- [ ] 레포 공개 범위 재확인 (Public 유지 여부)

---

## 9. 앞으로의 활용 제안

### 9.1 다시 돌리기만 하면 되는 경우 (0.5일)
