...
> 캐릭터, 배경, 소품 등 · 최대 10장
> ```
> 실존 인물 사진은 Claude 단계에서 분석이 거부될 수 있습니다.
...
| 그림체 추가 | `core/enums.py` `ArtStyle` + `onegrid_prompt_adapter.py`의 태그 2종 + 프론트 `storyboardFields.ts` |
...
| 🟡 | 12 | 미확정 스펙 | `role`은 `user`만 · 보존기간 삭제 미자동화 |
